...
// Ambil data jam kerja dengan JOIN ke tabel cabang
$sql = "SELECT jk.id, jk.nama_shift, jk.jam_masuk_akhir, jk.jam_pulang, c.nama_cabang, c.id as id_cabang
        FROM jam_kerja jk
        JOIN cabang c ON jk.id_cabang = c.id
        ORDER BY c.nama_cabang, jk.jam_masuk_akhir";
$result = $conn->query($sql);

// Ambil data cabang untuk dropdown
$cabangs = $conn->query("SELECT * FROM cabang ORDER BY nama_cabang ASC");
?>

<!-- Top Action Bar -->
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
    <div>
        <h2 class="text-2xl font-bold text-slate-800 dark:text-white">Pengaturan Jam Kerja</h2>
        <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Kelola aturan jam masuk dan pulang untuk setiap shift di masing-masing cabang.</p>
    </div>
    ...
    <div class="flex items-center gap-3 w-full sm:w-auto">
        <button onclick="openModal('modal-tambah')" class="flex items-center justify-center gap-2 px-4 py-2.5 bg-brand-600 hover:bg-brand-700 text-white rounded-xl transition-colors font-medium text-sm shadow-sm shadow-brand-500/30 w-full sm:w-auto whitespace-nowrap">
            <i class="fa-solid fa-plus"></i> Aturan Baru
        </button>
    </div>
</div>
...
<div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden flex flex-col mb-8">
    ...
    <div class="overflow-x-auto w-full">
        <table class="w-full text-left border-collapse" id="jamKerjaTable">
            <thead class="sticky top-0 z-10">
                <tr class="bg-slate-50 dark:bg-slate-900 text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wider border-b border-slate-200 dark:border-slate-700 shadow-sm">
                    ...
                    <th class="px-6 py-4 font-semibold">Nama Shift</th>
                    <th class="px-6 py-4 font-semibold text-center">Batas Jam Masuk</th>
                    ...
                    <?php $no = 1; while($row = $result->fetch_assoc()): ?>
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors group">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-600 dark:text-slate-300 search-target">
                                <?php echo htmlspecialchars($row['nama_shift']); ?>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400 border border-rose-200 dark:border-rose-800/50">
                                    <i class="fa-regular fa-clock"></i> <?php echo date('H:i', strtotime($row['jam_masuk_akhir'])); ?>
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-fuchsia-100 text-fuchsia-700 dark:bg-fuchsia-900/30 dark:text-fuchsia-400 border border-fuchsia-200 dark:border-fuchsia-800/50">
                                    <i class="fa-solid fa-house"></i> <?php echo date('H:i', strtotime($row['jam_pulang'])); ?>
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                                    <button onclick="openEditJamKerjaModal('<?php echo $row['id']; ?>', '<?php echo $row['id_cabang']; ?>', '<?php echo htmlspecialchars(addslashes($row['nama_shift'])); ?>', '<?php echo $row['jam_masuk_akhir']; ?>', '<?php echo $row['jam_pulang']; ?>')" class="p-2 text-brand-600 hover:bg-brand-50 rounded-lg dark:text-brand-400 dark:hover:bg-brand-900/30 transition-colors" title="Edit Data">
                                        <i class="fa-solid fa-pen-to-square"></i>
                                    </button>
                                    <a href="master_process.php?hapus_jam_kerja=<?php echo $row['id']; ?>" onclick="event.preventDefault(); handleDeleteAction(this.href, 'Hapus Jam Kerja?', 'Yakin ingin menghapus aturan jam kerja ini?');" class="p-2 text-red-600 hover:bg-red-50 rounded-lg dark:text-red-400 dark:hover:bg-red-900/30 transition-colors" title="Hapus">
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="6" class="px-6 py-8 text-center text-slate-500 dark:text-slate-400">
                            ...
                            <p>Belum ada aturan jam kerja yang dibuat.</p>
                        </td>
                    </tr>
                <?php endif; ?>
            </tbody>
    ...
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
        <div class="relative bg-white dark:bg-slate-800 rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-md w-full border border-slate-200 dark:border-slate-700">
            <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50">
                <h3 class="text-lg font-bold text-slate-800 dark:text-white">Tambah Aturan Jam Kerja</h3>
            ...
            <form action="master_process.php" method="POST">
                <div class="px-6 py-5 space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Pilih Cabang <span class="text-red-500">*</span></label>
                        <select name="id_cabang" required class="w-full px-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl text-slate-800 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 transition-colors appearance-none">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="text-xs text-slate-500 mt-1">Batas toleransi absen masuk</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Jam Pulang Standar <span class="text-red-500">*</span></label>
                            <input type="time" name="jam_pulang" required class="w-full px-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl text-slate-800 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 transition-colors">
                        </div>
                    </div>
                </div>
                <div class="px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-200 dark:border-slate-700 flex justify-end gap-3">
                    ...
                    <button type="submit" name="tambah_jam_kerja" class="px-6 py-2 bg-brand-600 hover:bg-brand-700 text-white rounded-xl shadow-sm shadow-brand-500/30 transition-colors text-sm font-medium">Simpan</button>
                </div>
            </form>
        </div>
    </div>
...
<div id="modal-edit" class="fixed inset-0 z-50 hidden">
    ...
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
        <div class="relative bg-white dark:bg-slate-800 rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-md w-full border border-slate-200 dark:border-slate-700">
            <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50">
                <h3 class="text-lg font-bold text-slate-800 dark:text-white">Edit Aturan Jam Kerja</h3>
            ...
            <form action="master_process.php" method="POST" id="form-edit">
                <input type="hidden" name="id_jam_kerja" id="edit-id-jam-kerja">
                <div class="px-6 py-5 space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Pilih Cabang <span class="text-red-500">*</span></label>
                        <select name="id_cabang" id="edit-id-cabang" required class="w-full px-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl text-slate-800 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 transition-colors appearance-none">
                            <option value="">-- Pilih Cabang --</option>
                            <?php 
                            $cabangs->data_seek(0);
                            while($c = $cabangs->fetch_assoc()): 
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Jam Pulang Standar <span class="text-red-500">*</span></label>
                            <input type="time" name="jam_pulang" id="edit-jam-pulang" required class="w-full px-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl text-slate-800 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 transition-colors">
                        </div>
                    </div>
                </div>
                <div class="px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-200 dark:border-slate-700 flex justify-end gap-3">
                    ...
                    <button type="submit" name="edit_jam_kerja" class="px-6 py-2 bg-brand-600 hover:bg-brand-700 text-white rounded-xl shadow-sm shadow-brand-500/30 transition-colors text-sm font-medium">Update</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
function openEditJamKerjaModal(id, cabang, shift, masuk, pulang) {
    ...
    document.getElementById('edit-id-jam-kerja').value = id;
    document.getElementById('edit-id-cabang').value = cabang;
    document.getElementById('edit-nama-shift').value = shift;
    document.getElementById('edit-jam-masuk-akhir').value = formatTime(masuk);
    document.getElementById('edit-jam-pulang').value = formatTime(pulang);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (editForm) {
        editForm.addEventListener('submit', function(e) {
            ...
            formData.append('edit_jam_kerja', '1'); 
                        ...
                        Swal.fire({
                            ...
                            fetch(window.location.href)
                                ...
                                .then(html => {
                                    ...
                                    document.querySelector('#jamKerjaTable tbody').innerHTML = doc.querySelector('#jamKerjaTable tbody').innerHTML;
