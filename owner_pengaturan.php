...
$sql_user = "SELECT nama, jenis_kelamin, foto_profil, ttd_path, stempel_path FROM users WHERE id = ?";
...
$stempel = $user_data['stempel_path'] ? 'assets/uploads/' . $user_data['stempel_path'] : null;
$foto_profil = $user_data['foto_profil'] ? 'assets/uploads/' . $user_data['foto_profil'] : 'https://ui-avatars.com/api/?name=' . urlencode($user_data['nama'] ?? 'Owner') . '&background=f43f5e&color=fff';
$nama = $user_data['nama'] ?? '';
...
<div class="max-w-5xl mx-auto mt-4 px-4 sm:px-0">
    <div class="mb-8">
        <h2 class="text-2xl font-bold text-slate-800 dark:text-white mb-2">Pengaturan Akun & Legalitas</h2>
        <p class="text-slate-500 dark:text-slate-400">Atur Tanda Tangan Digital dan Stempel Perusahaan untuk pengesahan dokumen.</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 flex flex-col">
            ...
            <div class="flex flex-col items-center mb-6">
                <div class="relative">
                    <div class="relative w-28 h-28 mb-3 group cursor-pointer rounded-full flex items-center justify-center overflow-hidden border-4 border-slate-100 dark:border-slate-700 shadow-sm" onclick="toggleDropdownFoto(event, 'dropdownFotoOwner')">
                    ...
                    <div id="dropdownFotoOwner" class="hidden absolute top-16 -right-16 w-32 bg-[#333333] rounded-xl shadow-xl z-50 overflow-hidden border border-gray-600 text-[13px]">
                        <button type="button" onclick="lihatFotoProfil('<?php echo htmlspecialchars($foto_profil); ?>'); hideDropdownFoto('dropdownFotoOwner');" class="w-full px-3 py-2.5 text-gray-200 hover:bg-[#444] text-center transition-colors">Lihat Foto</button>
                        <div class="border-t border-gray-600 mx-3"></div>
                        <button type="button" onclick="document.getElementById('input_foto_profil').click(); hideDropdownFoto('dropdownFotoOwner');" class="w-full px-3 py-2.5 text-gray-200 hover:bg-[#444] text-center transition-colors">Ganti Foto</button>
                    </div>
                </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 flex flex-col items-center">
            ...
            <div class="relative w-full h-40 mb-4 group cursor-pointer border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-xl flex items-center justify-center overflow-hidden bg-slate-50 dark:bg-slate-900/50" onclick="document.getElementById('input_ttd').click();">
                <?php if ($ttd): ?>
        ...
        <!-- Stempel Card -->
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 flex flex-col items-center">
            <h3 class="text-lg font-bold text-slate-800 dark:text-white mb-4 border-b w-full pb-2 text-center">Stempel Perusahaan</h3>
            
            <div class="relative w-full h-40 mb-4 group cursor-pointer border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-xl flex items-center justify-center overflow-hidden bg-slate-50 dark:bg-slate-900/50" onclick="document.getElementById('input_stempel').click();">
                <?php if ($stempel): ?>
                    <img id="preview_stempel" src="<?php echo htmlspecialchars($stempel); ?>" alt="Stempel" class="h-full object-contain p-2 relative z-10">
                <?php else: ?>
                    <div id="placeholder_stempel" class="text-slate-400 flex flex-col items-center">
                        <i class="fa-solid fa-stamp text-4xl mb-2"></i>
                        <span class="text-sm">Upload Stempel (PNG)</span>
                    </div>
                <?php endif; ?>
            ...
            <form id="form-stempel" enctype="multipart/form-data">
                <input type="file" id="input_stempel" name="stempel" accept="image/png" class="hidden" onchange="uploadFile('stempel')">
            </form>

            <?php if ($stempel): ?>
            <button onclick="hapusFile('stempel')" class="px-4 py-2 bg-rose-50 text-rose-600 hover:bg-rose-500 hover:text-white rounded-lg transition-colors text-sm font-semibold w-full">
                <i class="fa-solid fa-trash mr-1"></i> Hapus Stempel
            </button>
            <?php endif; ?>
        </div>
    </div>
...
<script>
    function uploadFile(type) {
        ...
        .then(data => {
            if (data.success) {
                let titleName = type === 'ttd' ? 'Tanda Tangan' : 'Stempel';
    ...
    function hapusFile(type) {
        let titleName = type === 'ttd' ? 'Tanda Tangan' : (type === 'stempel' ? 'Stempel' : 'Foto Profil');
        Swal.fire({
